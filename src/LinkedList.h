...
public:
    ...
    static LinkedList addFactorialToLinkedList(int num);
    static BigInt fibonnacci(int num);
    static LinkedList<T> addFibonnacciToLinkedList(int n);
...
template<class T>
LinkedList<T> LinkedList<T>::addFactorialToLinkedList(int num) {
    ...
    for (int i = 0; i < round(sub.size() / 3); i++) {
...
template<class T>
LinkedList<T> LinkedList<T>::addFibonnacciToLinkedList(int n) {
    auto *ll = new LinkedList<Array<Integer>>();
    auto *array = new Array<Integer>(3);
    Integer *integer;
    string sub = "";
